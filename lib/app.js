...
module.exports.initialize = function (config) {
  ...
  app.use(express.static(path.join(__dirname + '/../', 'node_modules'))) // eslint-disable-line no-path-concat
  ...
  // use multer middleware upload image file
  var uploadFilePath = app.locals.config.get('features').uploadFilePath;
  if (uploadFilePath) {
    app.use('/upload', express.static(uploadFilePath)); // eslint-disable-line no-path-concat
    app.use('/uploadRouter', require('../routes/upload'));
  }
